...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    shifted = [val + epsilon for idx, val in enumerate(vals) if idx == arg]
    return (f(*shifted) - f(vals)) / epsilon
...
class Mul(ScalarFunction):
    ...
    def backward(ctx, d_output):
        ...
        return operators.mul(d_output, b), operators.mul(a, d_output)
...
class Exp(ScalarFunction):
    ...
    def forward(ctx, a):
        ctx.save_for_backward(a)
        return operator.exp(a)

    @staticmethod
    def backward(ctx, d_output):
        a = ctx.saved_values
        return d_output * operator.exp(a)
